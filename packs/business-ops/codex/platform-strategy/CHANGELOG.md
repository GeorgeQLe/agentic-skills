...
- Added `briefing-slides` to the required conventions via the shared packaged convention resolver. Dense alignment/interrogation pages remain source artifacts, while `briefing-slides/platform-strategy-{topic}.html` is now the primary review surface and compiled YAML routes back to `$platform-strategy`.
...
- Route foundational and new-audience discovery handoffs to `$customer-discovery` instead of the retired `$icp` executable.
