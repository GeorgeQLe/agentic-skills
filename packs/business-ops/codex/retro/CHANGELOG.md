...
- Added `briefing-slides` to the required conventions via the shared packaged convention resolver. Dense alignment/interrogation pages remain source artifacts, while `briefing-slides/retro-{topic}.html` is now the primary review surface and compiled YAML routes back to `$retro`.
...
- Route wrong-ICP follow-up discovery to `$customer-discovery` instead of the retired `$icp` executable.
