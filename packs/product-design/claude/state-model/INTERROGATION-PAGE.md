...
**Archiving.** Round pages are durable artifacts; do not overwrite a prior round. Each round gets its own `r{N}` file. If a round page must be rebuilt before its answers are consumed (e.g. the user asks for a different question set), archive the prior copy to `docs/history/archive/YYYY-MM-DD/HHMMSS/interrogation/state-model-r{N}-{branch}.html` before replacing it.
