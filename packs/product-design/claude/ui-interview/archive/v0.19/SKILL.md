...
5b. **Requirements gate (requirements-only mode)**
   ...
   - Only after final compiled YAML approval, write `specs/ui-requirements-[topic].md` (content requirements) and `ui-requirements-[topic]-interview.md` (interview log), archive the working packet, and convert the page to `confirmed` per step 9.
