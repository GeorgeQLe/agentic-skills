...
- `Session guidance: continue in a fresh Codex session, then run the Exact next command below. If using the ## Invoke With YAML block, paste it only into that fresh context alongside the command; it gives the cold agent routing context, while the command and durable cursor remain authoritative. Staying in this session is exceptional and allowed only for small folded runs where context is still clearly sufficient.`
