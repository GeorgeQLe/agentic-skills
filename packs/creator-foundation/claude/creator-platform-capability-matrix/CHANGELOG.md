...
- Added `briefing-slides` to the required conventions via the shared packaged convention resolver. Dense alignment/interrogation pages remain source artifacts, while `briefing-slides/creator-platform-capability-matrix-{topic}.html` is now the primary review surface and compiled YAML routes back to `/creator-platform-capability-matrix`.
...
- Added npm-aware install-route wording to the Pack Availability Guard while preserving the Claude in-agent `/pack install <pack>` route.
