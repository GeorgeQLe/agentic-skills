...
- Added `briefing-slides` to the required conventions via the shared packaged convention resolver. Dense alignment/interrogation pages remain source artifacts, while `briefing-slides/creator-presence-dossier-{topic}.html` is now the primary review surface and compiled YAML routes back to `$creator-presence-dossier`.
