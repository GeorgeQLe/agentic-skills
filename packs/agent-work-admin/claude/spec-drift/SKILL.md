...
Checks that specs and codebase tell the same story. Extracts verifiable claims from spec documents, checks each against the actual implementation, and flags divergence. Think of it as a linter for spec-to-code fidelity — complementary to `/reconcile-research` (business-discovery pack) (doc-to-doc) and `/expert-review` (broad code review).
...
Follow the shared Alignment Page convention in CLAUDE.md. Output: `alignment/spec-drift-{topic}.html`.
