# Interrogation Page — devtool-monetization

When this skill needs to elicit user context before producing research or other durable output, run the **stage-zero interrogation loop** in HTML before advancing to stage one (the skill's first post-interrogation stage). Build one interrogation page per round at `interrogation/devtool-monetization-r{N}-{branch}.html`, where `{N}` is the 1-based round number and `{branch}` is the research fork or product-path slug (in the flat single-product case `{branch}` is the normalized topic slug, matching the alignment page's `{topic}`).
...
```
stage 0  →  interrogation/devtool-monetization-r1..rN.html   ELICITATION: open input drives research
...
stage 1  →  skill's first post-interrogation stage    SELECTION: options informed by stage 0
            research runs
stage 2  →  alignment/devtool-monetization-{topic}.html        REVIEW/APPROVAL of deliverables
```
...
```
round = 1
build interrogation/devtool-monetization-r{round}-{branch}.html   (manifest if round==1, else follow-ups)
...
interrogation_page: interrogation/devtool-monetization-r1-{branch}.html
agent_routing:
  workflow: interrogation-loop
  parent_skill: devtool-monetization
  command: "<parent-skill-command>"
  gate_owner: parent-orchestrator
  gate_type: interrogation-round
  round: 1
  answer_sidecar: research/_working/interrogation-devtool-monetization-r1.yaml
