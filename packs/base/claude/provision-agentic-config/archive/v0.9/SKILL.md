...
2. **Insert the orchestration blocks:**
   ...
   - Each block begins with `<!-- provision-agentic-config v0.9 -->`. When replacing an existing block, update this comment to the current version. The `/sync` skill uses this comment to detect stale provisioning.
