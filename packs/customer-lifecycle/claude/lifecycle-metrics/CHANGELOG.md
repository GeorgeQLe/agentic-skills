...
- Added `briefing-slides` to the required conventions via the shared packaged convention resolver. Dense alignment/interrogation pages remain source artifacts, while `briefing-slides/lifecycle-metrics-{topic}.html` is now the primary review surface and compiled YAML routes back to `/lifecycle-metrics`.
...
- Scope lifecycle metrics to active product path by default
