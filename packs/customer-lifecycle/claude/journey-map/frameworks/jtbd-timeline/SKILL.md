...
1. **Stage 1 - Research and clarify.** Perform the research, run required source/code checks, and ask any needed clarification questions. Write only a non-canonical working packet: flat mode uses `research/_working/preliminary-jtbd-timeline-research.md`; product-path mode uses `research/{slug}/_working/preliminary-jtbd-timeline-research.md`. Do not create or update canonical research, spec, or task files in Stage 1.
