# customer-journey-canvas changelog (claude)

## v0.11 - 2026-07-06

- Added `briefing-slides` to the required conventions via the shared packaged convention resolver. Dense alignment/interrogation pages remain source artifacts, while `briefing-slides/customer-journey-canvas-{topic}.html` is now the primary review surface and compiled YAML routes back to `/customer-journey-canvas`.
...
- Initial version — Stickdorn customer journey canvas with stage×touchpoints×actions×emotion×backstage×pain×opportunities grid
