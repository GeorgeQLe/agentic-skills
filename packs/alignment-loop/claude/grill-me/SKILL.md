...
Relentless one-question-at-a-time grilling to build shared understanding before any code or docs.

## Process

1. **Gather context silently.** Read `.agents/project.json`, `README.md`, `CLAUDE.md`, and any existing `research/` or `specs/` files relevant to the topic.
2. **Explore codebase via subagent.** Gather grounding facts (stack, structure, relevant patterns). Do not surface exploration results to the user.
3. **Begin grilling.** Use `$ARGUMENTS` as the seed topic.
...
Stop grilling when any one condition is met:
