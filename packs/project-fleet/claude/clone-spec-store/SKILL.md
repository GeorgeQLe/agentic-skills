...
Default arguments (ask the user only if missing):
...
- `--owner`: GitHub owner (user or org) for downstream repos. Required before Phase 6 execution.
...
1. Create repo scaffold:
   - `CLAUDE.md` — project conventions (copy structure below).
...
3. Create Draft 0 specs under `specs/batch-XX/NNN-<slug>.md` (20 per batch). One H1, one-paragraph summary, placeholder sections — enough to reserve IDs.
...
## Goals
## Non-Goals
## Research Sources        (table: Source | Exact URL | Evidence Used | Status)
## Detailed Design
## Core User Journeys
## Screen Inventory        (or "Surface Inventory" for non-GUI domains; table: Surface | Purpose | Inputs | Required States | Edge/Failure)
## Data Model
## API And Backend Contracts
## Realtime, Push, And Offline Behavior   (or "Runtime And Offline Behavior")
...
- **CLI tools / devtools / libraries**: rename "Screen Inventory" → "Command/Surface Inventory"; replace "Push/Offline" with "Runtime/Error-Handling Behavior"; add "CLI Contracts" (flag/subcommand matrix) and "Install/Distribution" sections.
...
1. Confirm `--domain`, `--count`, `--owner`, `--repo-slug-suffix`. Ask for any missing required values.
...
- `CLAUDE.md`, root `README.md`, `LICENSE`, `CONTRIBUTING.md`, `SECURITY.md`
- `tasks/ideas.md`, `tasks/roadmap.md`, `tasks/repo-seeding.md`, `tasks/history.md`, `tasks/todo.md`, `tasks/spec-quality-audit.md`
- `specs/batch-NN/NNN-<slug>.md` (20 per batch by convention; adjust if N ≠ 100)
...
If accessible, read those files for the exact wording and reuse structure verbatim with domain-appropriate substitutions.
...
When this skill prepares approval-gated durable planning, research, spec, task, prototype, report, or document deliverables, build a custom HTML alignment preview page at `alignment/clone-spec-store-{topic}.html` before asking the user to approve the canonical write; treat the HTML page as a review preview, not the canonical synthesized deliverable. Point the user to the page, summarize the recommended output and file changes, ask what questions or adjustments they have, and write or update the canonical files only after approval. For non-approval durable output writes, also build the custom HTML alignment page at `alignment/clone-spec-store-{topic}.html`. Use a normalized topic slug from the app, feature, research subject, report subject, or output filename. If the skill writes multiple scoped deliverables in one run, either write one alignment page per scope or one overview page that links each scope. Before replacing an existing alignment page, archive it to `docs/history/archive/YYYY-MM-DD/HHMMSS/alignment/clone-spec-store-{topic}.html`. Attempt to open the resulting HTML page in the browser and report whether the browser open succeeded or was blocked. A blocked browser-open attempt does not make the skill fail when the files were written correctly.
