...
The broader creator-media workflow now lives in `creator-foundation` and `youtube-ops`. Install those packs with `remotion` when a project needs the full path from creator strategy to Remotion production:
