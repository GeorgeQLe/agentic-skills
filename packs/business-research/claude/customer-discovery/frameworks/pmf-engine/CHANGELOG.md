# pmf-engine changelog (claude)

## v0.11 - 2026-07-06

- Added `briefing-slides` to the required conventions via the shared packaged convention resolver. Dense alignment/interrogation pages remain source artifacts, while `briefing-slides/pmf-engine-{topic}.html` is now the primary review surface and compiled YAML routes back to `/pmf-engine`.
...
- Initial version: Vohra/Supan PMF Engine for product-exists customer discovery, covering Sean Ellis PMF signal, very-disappointed segmentation, behavioral evidence checks, High-Expectation Customer synthesis, target segment recommendation, and data-readiness gaps.
