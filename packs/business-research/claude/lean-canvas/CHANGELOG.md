# lean-canvas changelog (claude)

## v0.12 - 2026-07-06

- Added `briefing-slides` to the required conventions via the shared packaged convention resolver. Dense alignment/interrogation pages remain source artifacts, while `briefing-slides/lean-canvas-{topic}.html` is now the primary review surface and compiled YAML routes back to `/lean-canvas`.
...
- Route back to ux-variations after business-model synthesis when upstream AFPS evidence exists.
