...
- Added `briefing-slides` to the required conventions via the shared packaged convention resolver. Dense alignment/interrogation pages remain source artifacts, while `briefing-slides/feature-pricing-matrix-{topic}.html` is now the primary review surface and compiled YAML routes back to `/feature-pricing-matrix`.
...
- Initial feature/pricing matrix framework subskill for `/competitive-analysis` orchestrator synthesis.
