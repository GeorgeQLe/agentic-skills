...
- Added `briefing-slides` to the required conventions via the shared packaged convention resolver. Dense alignment/interrogation pages remain source artifacts, while `briefing-slides/four-forces-{topic}.html` is now the primary review surface and compiled YAML routes back to `$four-forces`.
...
- Initial version: Moesta Four Forces of Progress — Push/Pull/Anxiety/Habit switching analysis, force balance scoring, and switching catalyst identification per ICP candidate
