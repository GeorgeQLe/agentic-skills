...
- DEFAULT: check for `product-design` pack → if enabled, recommend `$user-flow-map [positioning-backed product direction]`; if not enabled, recommend `$pack install product-design`
