...
- Added `briefing-slides` to the required conventions via the shared packaged convention resolver. Dense alignment/interrogation pages remain source artifacts, while `briefing-slides/gtm-{topic}.html` is now the primary review surface and compiled YAML routes back to `/gtm`.
...
- Scope GTM strategy to active product path by default
- Surface product path implications when GTM insights affect deferred paths
