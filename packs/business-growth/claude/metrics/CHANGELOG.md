...
- Added `briefing-slides` to the required conventions via the shared packaged convention resolver. Dense alignment/interrogation pages remain source artifacts, while `briefing-slides/metrics-{topic}.html` is now the primary review surface and compiled YAML routes back to `/metrics`.
...
- Scope metrics framework to active product path by default
- Surface product path implications when metrics insights affect deferred paths
