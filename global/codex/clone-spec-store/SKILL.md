...
description: Build a canonical lawful-functional-parity spec store for N software ideas in any domain, then seed one private downstream implementation repo per idea.
type: planning
version: 1.0.0
...
Pilot one app first (for example, the most architecturally-teaching example), then extend the same structure to the rest in batches. The pilot becomes the reference template for the other specs.
...
If `--count` is not 100, create a row for every requested ID and title the phase accordingly.
