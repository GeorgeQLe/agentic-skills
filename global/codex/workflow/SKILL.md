...
1. Check `.agents/project.json`.
2. If it declares `project_type: "game"`, use the game workflow graph and recommend `$game-workflow` when game research artifacts are missing.
3. If it declares `project_type: "devtool"`, use the devtool workflow graph and recommend `$devtool-workflow` when devtool research artifacts are missing.
4. If it declares `project_type: "business-app"` or is missing, use the business-app graph below.
5. If a needed domain skill is unavailable, recommend `$pack install <pack>` instead of assuming it exists globally.
