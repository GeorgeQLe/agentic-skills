...
6. Recommend strategy: `agent-team` (independent packages), `serial` (linear chain), or mixed with dependency edges.
7. Generate lane specs in execution profile format:
   ...
   - **Package lanes:** one per package, `Owns` scoped to package dir, `Must not edit` includes all lockfiles + root configs + other lanes' paths. Scope text includes "Do NOT run pnpm install or any lockfile-modifying command."
8. Validate: disjoint Owns, all lockfiles excluded from write lanes, valid DAG.
...
- **Lane Specifications**: in execution profile format (Agent, Role, Mode, Scope, Owns, Must not edit, Depends on, Deliverable)
