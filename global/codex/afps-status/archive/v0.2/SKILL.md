...
6. Choose the next route with these rules:
   ...
   - Concept exists but business discovery is missing: `$pack install business-discovery` inside Codex, or `npx skillpacks install business-discovery` from the project shell.
...
7. Validate command availability before recommending pack-local routes:
   ...
   - If the best command lives in an uninstalled pack, recommend the pack install command instead of the unavailable command: `$pack install <pack>` inside Codex, or `npx skillpacks install <pack>` from the project shell.
