...
1. **Read session history:**
   - Read `~/.claude/history.jsonl` which contains all user messages across all sessions. Each line is JSON with fields: `display` (user message), `timestamp`, `project`, `sessionId`, `pastedContents`.

2. **Process and analyze:**
   - Use Python to process the file efficiently. Extract and analyze:
