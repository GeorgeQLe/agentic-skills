...
Run these steps before the main process. If any step fails, warn the user and continue without kanban — the brainstorm output to `tasks/ideas.md` must always succeed.
...
1. For each idea in the output (each `- **Title** — description` bullet):
   ...
   b. Search the board for a card with the same name: `search --query "<title>"`
