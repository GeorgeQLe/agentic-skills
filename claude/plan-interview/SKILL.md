...
1. **Check for ICP context:**
   - If `research/icp.md` exists, read it and treat it as foundational context. Ground solution decisions against the ICP: does this feature serve the user journey? Does this architecture match the user's technical sophistication? Does this UX match how the customer provisions and onboards?
...
2. **Conduct the interview:**
   ...
   - When a decision point genuinely has multiple viable approaches, list each option with a clear rationale and provide a simple pros and cons comparison. State your recommendation and explain why. For the recommended option, explain how the con can be mitigated if feasible. Only present options when distinct alternatives genuinely exist — do not manufacture choices.
