...
function staleProjectPackError(pack, location, reason = '') {
  ...
  return new Error(
    `Stale pack entry '${pack}' in .agents/project.json ${location}.${detail} ` +
      `Run 'npx @glexcorp/gskp remove ${pack}' or edit .agents/project.json to remove or rename it.`
  );
}
...
export async function doctorProject({ manifest, projectRoot = process.cwd(), args = [] } = {}) {
  ...
  if (anyStale) {
    console.log('');
    console.log('Fix: npx @glexcorp/gskp refresh (or scripts/pack.sh refresh from a source checkout)');
    return 1;
  }
