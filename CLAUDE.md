...
- **Skillpacks Manifest Is Index-Generated**: `packages/skillpacks/dist/skillpacks-manifest.json` (and the `build-package.mjs` source fingerprint) are generated from the **git index**, not the working tree. Regeneration therefore never captures another session's unstaged edits on the shared tree. Workflow: `git add` your skill edits **before** running `npm run build` (or `node packages/skillpacks/scripts/build-skillpacks-manifest.mjs`), then `git add` the regenerated manifest and commit source + manifest **together** in one atomic commit. `build:check` validates the committed manifest against the index, so a clean checkout (index == HEAD) always passes. The two **skills-showcase generators** (`apps/skills-showcase/scripts/generate-skills-showcase-data.mjs` and `generate-skills-showcase-github-data.mjs`) follow the same rule for their **tracked** inputs — skill/pack metadata, benchmark markdown, proof artifacts, `tasks/history.md`, and the embedded source fingerprints all read from the **git index**, not the working tree, so a concurrent session's unstaged source edits can't leak into committed `skills-data.js` / `github-proof-data.js`. The untracked (gitignored) benchmark-run evidence under `tests/benchmarks/runs/**` intentionally still reads from the working tree. Same stage-before-build workflow: `git add` your source edits before regenerating, then commit source + regenerated assets together.
...
1. If `alignment.build_in_public === true` → BIP already on, skip only the enablement prompt and continue the skill's enabled-BIP behavior. In a wrap/ship context, use saved `alignment.bip_platforms` and draft exhaustive phase-aware, source-safe BIP post candidate batches per platform, or explain why no safe public angle exists; do not report only that the BIP gate was skipped. If no platforms are saved, ask one project-platform setup question and persist the answer with `scripts/pack.sh set-bip-platforms <platform...>` or the `npx` equivalent before drafting. At kickoff, enabled BIP flows into the skill's existing alignment-page BIP behavior.
...
3. Else ask once, concisely: explain that BIP generates source-safe social posts from alignment pages / shipped work, and ask whether to enable it for this project.
   - **Yes:** run `scripts/pack.sh set-bip on` (or `npx skillpacks set-bip on`) **and** `scripts/pack.sh set-bip-prompt dismiss` (or the `npx` equivalent) so a later `set-bip off` won't re-trigger the prompt. Then run the same enabled-BIP behavior for the skill: in a wrap/ship context, use saved platforms or collect and persist one platform setup, then draft exhaustive phase-aware, source-safe BIP post candidate batches about what just shipped, following `docs/social-ledger-convention.md`; at kickoff, enabling flows into the skill's existing alignment-page BIP behavior with no separate post offer.
