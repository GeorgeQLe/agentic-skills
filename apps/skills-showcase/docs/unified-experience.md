...
3. **`/card/[id]`** — intercepting + standalone pair + `@modal/default.tsx`. Additive. _Shipped (2026-06-19):_ server-side data loader `src/server/skillsData.ts` (parses `window.SKILLS_SHOWCASE_DATA` off disk; `dedupeCards` path-sorted claude-wins, `getCardById`, `decksForCard`); shared `CardDetail` renderer (enlarged flip + always-rendered stats: per-agent pass rates + A/B/C grade + `/deck/<slug>` chips); standalone `app/card/[id]/page.tsx` (`dynamicParams=false`, `generateStaticParams` over the 196 deduped ids, so codex variants + unknown ids 404 — no duplicate-content); `@modal/(.)card/[id]` intercept + `@modal/default.tsx` (null) + `@modal/layout.tsx` (scoped Tailwind) + root-layout `modal` slot; `CardDetailModal` overlay (scrim/Escape/scrim-click/mobile swipe-down, reduced-motion instant). Info-vs-collect split: expand controls on builder fan cards (`PackOpener` `onExpand`, `stopPropagation`) + filled slot cards route via `useRouter().push` (intercepted → modal) without collecting. Deck pushState morph untouched (separate subtree). Typecheck clean; build 196 SSG card pages; Vitest 177/177; Playwright 22/22 (deck-morph 18 unchanged + 4 new). Adversarial review fixed the codex-id duplicate-content gap (added `dynamicParams=false`).
