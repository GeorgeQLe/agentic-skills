...
4. **Completion + output** — deck-complete sequence, CLI/`project.json`/share emission, custom-deck variant rules.
   ...
   - _Deferred to follow-on:_ the per-slot collapse-to-target gather (thin cut gathers the panel, not each slot), custom-deck explicit-pack-list output + overlay install lines, custom-deck variant rules.
