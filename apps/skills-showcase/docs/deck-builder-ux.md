...
4. **Completion + output** — deck-complete sequence, CLI/`project.json`/share emission, custom-deck variant rules.
   - _Deck-complete output panel shipped (2026-06-19):_ when every `deck.skills` card settles, `BuilderPanel` derives `deckComplete` (off `settledCount`, no new state) and mounts the new `DeckCompletionPanel` after the head. The panel gathers in (spring scale, the collapse-to-target read) and flips (the `SkillCard` card-flip idiom — `rotateY` + `backface-visibility`, scaled up; "promote, don't rewrite") to reveal its back: the canonical `npx skillpacks install-deck <slug>` command, a `project.json` download (`buildDeckProjectJson` mirrors the `.agents/project.json` shape — `enabled_packs` from the deck's unique packs + `skill_pack_version` + `deck` metadata, per `docs/skillpacks-npm-distribution.md`), a share affordance (best-effort clipboard of the `/deck/<slug>` URL), and a "keep editing" dismiss (re-arms per deck — `BuilderPanel` is keyed by slug). The back face is always in the DOM (hidden by backface) so the output is testable independent of the flip frame. Reduced motion skips the gather/flip and shows the output immediately (§E). The always-visible CLI panel (Phase 3) stays as the precursor; this is additive and `/prototype` is unaffected. Covered by `DeckTableShell.test.tsx` (+ `buildDeckProjectJson` unit) + `e2e/deck-table-shell.spec.ts`.
   - _Per-slot collapse-to-target gather shipped (2026-06-19):_ on the completion edge (`deckComplete`), `BuilderPanel` now launches a **gather** before the flip — promoting the existing `FlightLayer`/`FlightClone`/`FlightRecord` clone machinery ("promote, don't rewrite"): a clone per settled slot card flies (existing `rectOf` + `FLIGHT_SPRING`, fixed body-portal overlay) into the completion stack's rect (new `data-completion-target` hook on `.deck-completion-card`), and only once the last gather clone lands does `revealCompletion` trigger the stack flip. The flip became parent-controlled — `DeckCompletionPanel` takes a `revealed` prop instead of its old internal 520 ms timer, so the gather→flip sequence is deterministic. A second `FlightLayer` (gather `onLand` → trigger flip) keeps the gather path from crossing the card-flight settle path; the one-shot `gatherFiredRef` reads `prefers-reduced-motion` directly (the shell's `matchMedia` state can lag the hydrated-mount edge), and reduced motion skips the gather and reveals immediately (§E). Driven in tests via the existing `__deckFlight` bridge extended with `gatherInFlight`/`landGather`. Additive; `/prototype` unaffected. Covered by `DeckTableShell.test.tsx` (gather-then-reveal + reduced-motion) + `e2e/deck-table-shell.spec.ts` (rAF sampler: gather clones paint and the reveal never precedes the gather). **§6 stack-then-flip sequence complete.**
   - _Deferred to follow-on:_ custom-deck explicit-pack-list output + overlay install lines, custom-deck variant rules.
