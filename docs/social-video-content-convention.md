...
This is the thin router and shared contract for build-in-public and alignment-producing agents that draft source-safe video ideas, outlines, hooks, and scripts. Load this file first, then load only the selected channel or prompt docs from `docs/social/` or `assets/social/`. This convention covers ideation and scripting guidance only; it does not authorize auto-publishing, video production, scraping, uploading, or account operations.
