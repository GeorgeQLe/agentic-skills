...
8. Default AFPS business-product route: `icp -> competitive-analysis -> journey-map -> positioning -> user-flow-map -> ui-interview --requirements-only -> ux-variations --layout-mode -> prototype -> uat --variant-evaluation -> consolidate-variations -> research-roadmap --post-prototype -> spec-interview -> research-roadmap --post-spec -> roadmap`. Optional research/framework detours are allowed only when their trigger would change positioning, product-loop direction, flow/design shape, or prototype choices.
...
| --- | --- | --- |
| Global planning/execution (`idea-scope-brief`, `user-flow-map`, `spec-interview`, `ux-variations`, `ui-interview`, `prototype`, `consolidate-variations`, `roadmap`, `plan-phase`, `run`, `ship`, `ship-end`) | Concept needed, flow map needed, requirements needed, layout variants needed, variant evaluation needed, prototype needed, execution ready, shipping ready, next phase ready, mode unset | Use mode-aware routing from `docs/operating-modes.md`: `claude-only` stays on Claude, `codex-only` stays on Codex, `hybrid` delegates from Claude to Codex, unset presents all viable paths. For user-facing product work, idea scope brief precedes ICP when no concept brief, downstream research, or specs exist; the canonical pipeline after positioning is: user-flow-map -> ui-interview --requirements-only -> ux-variations --layout-mode -> prototype -> uat --variant-evaluation -> consolidate-variations -> research-roadmap --post-prototype -> spec-interview -> research-roadmap --post-spec -> roadmap. Built UI variants must route through prototype and UAT/evaluation evidence before `consolidate-variations`; consolidation is valid only after evaluation evidence exists or the user explicitly says they reviewed variants and are ready to converge. |
...
| `competitive-analysis` standard mode | Journey missing, positioning missing, flow map missing, contested solution fit, GTM missing, codebase exists | Missing journey is first: if `customer-lifecycle` is unavailable, recommend `pack install customer-lifecycle`; if it is already enabled, recommend `journey-map`. Then route to `positioning`, `user-flow-map`, optional `value-prop-canvas` only for contested solution-fit evidence, `gtm`, `mvp-gap`. |
...
| `customer-feedback` | Major impact, stale ICP, stale journey, new needs, no stale findings, experiment feedback | Major -> `reconcile-research`. Stale ICP -> `icp`. Stale journey -> `journey-map`. New needs -> `brainstorm` or `ux-variations`. No stale findings -> `research-roadmap`. Experiment feedback -> `assumption-tracker`. |
...
| `positioning` (market mode — framework selection) | Framework selection complete, journey missing | After framework selection: write selected frameworks to `tasks/todo.md` for `/exec` execution. Missing journey -> `journey-map` before framework selection. No downstream routing until synthesis. |
| `positioning` (synthesis mode) | Synthesis complete, flow map missing, contested solution fit, business-model risk, GTM missing, GTM exists, monetization missing, codebase exists, major downstream conflict | Major -> `reconcile-research`. Missing journey -> `journey-map`. Default -> `user-flow-map` when `product-design` is enabled, otherwise `pack install product-design`. Include `value-prop-canvas` only for weak/disputed fit, `lean-canvas` only for revenue/channel/cost/defensibility risks, `monetization` only when pricing is central to positioning. |
| `positioning` (product mode) | Framework selection complete (obviously-awesome queued), synthesis complete | Same as synthesis mode end states after synthesis completes. Framework selection writes single obviously-awesome step to `tasks/todo.md`. |
...
| `pmf-assessment` | Weak PMF, moderate PMF, strong PMF, pre-launch | Weak -> skill addressing weakest area (`customer-feedback`, `icp`). Moderate -> `growth-model` or `hook-model`. Strong -> `growth-model` or `roadmap`. Pre-launch -> design framework for future use. |
| `platform-strategy` | Top expansion candidate, enterprise target, no assumption tracker, specs exist | Recommend `experiment [top candidate]` because expansion must be validated before build. Include `enterprise-icp`, `assumption-tracker`, `competitive-analysis [adjacent category]`, `icp [new audience]`, or `roadmap` only when their conditions hold. |
