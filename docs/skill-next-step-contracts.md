...
| --- | --- | --- |
| Global planning/execution (`concept-exploration`, `spec-interview`, `ux-variation`, `ui-interview`, `ui-consolidate`, `roadmap`, `plan-phase`, `run`, `ship`, `ship-end`) | Concept needed, plan needed, design planning needed, variant evaluation needed, execution ready, shipping ready, next phase ready, mode unset | Use mode-aware routing from `docs/operating-modes.md`: `claude-only` stays on Claude, `codex-only` stays on Codex, `hybrid` delegates from Claude to Codex, unset presents all viable paths. For user-facing product work, concept exploration precedes ICP when no concept brief, downstream research, or specs exist; roadmap is valid only after journey, UX variation, and UI interview artifacts exist or are explicitly not applicable. Built UI variants must route through UAT/evaluation evidence before `ui-consolidate`; consolidation is valid only after evaluation evidence exists or the user explicitly says they reviewed variants and are ready to converge. |
