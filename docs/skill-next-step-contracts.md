...
| Global project operations (`sync`, `deploy`, `release`, `decommission`, `migrate`, `scaffold`, `pack`, `install-workflow-orchestration`, `commit-and-push-by-feature`, `handoff`, `guide`, `uat-guide`, `analyze-sessions`) | Completed, blocked, needs confirmation, no-op | Next step must be an operational continuation of the completed state. Confirmation-only states must ask for confirmation, not recommend an automatic command. |
