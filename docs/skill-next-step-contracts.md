...
| `gtm` | Major downstream conflict, growth-model missing, roadmap missing, metrics missing, open research question, roadmap exists | Major -> `reconcile-research`. Growth-model missing -> `growth-model`. Roadmap missing with specs -> `roadmap`. Metrics missing -> `metrics`. Open question -> `spec-interview [topic]`. Roadmap exists -> `run`. |
...
| `value-prop-canvas` | Positioning missing, gaps identified, competitive-analysis missing, positioning exists | `positioning` is the natural next step. If gaps identified -> `spec-interview [top gap]`. If no competitive-analysis -> `competitive-analysis`. If positioning exists -> `lean-canvas`. |
| `lean-canvas` | Journey-map missing, monetization missing, positioning missing, riskiest hypothesis | `journey-map` is always the natural next step. Include `monetization` to validate revenue/cost hypotheses, `spec-interview [riskiest hypothesis]`, or `positioning` only when conditions hold. |
| `hook-model` | B2B/enterprise (skippable), metrics missing, engagement mechanism, monetization missing | B2B -> offer skip to `metrics`. Otherwise `metrics` is always the next step. Include `spec-interview [engagement mechanism]` or `monetization` when conditions hold. |
| `growth-model` | Top growth mechanism, specs exist, live product | `spec-interview [top growth mechanism]` is the recommended next step. Include `roadmap` when specs exist, `experiment [growth hypothesis]` for live products. |
