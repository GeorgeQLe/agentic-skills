...
| `competitive-analysis` standard mode | Journey missing, value-prop missing, specs missing, GTM missing, codebase exists | Missing journey is first: if `customer-lifecycle` is unavailable, recommend `pack install customer-lifecycle`; if it is already enabled, recommend `journey-map`. Then route to `value-prop-canvas`, `spec-interview`, `gtm`, `mvp-gap`. |
