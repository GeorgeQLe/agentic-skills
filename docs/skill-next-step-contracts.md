...
| `competitive-analysis` standard mode | Journey missing, value-prop missing, specs missing, GTM missing, codebase exists | `journey-map` is the first standard-mode recommendation when `research/journey-map.md` is missing; then `value-prop-canvas`, `spec-interview`, `gtm`, `mvp-gap`. |
