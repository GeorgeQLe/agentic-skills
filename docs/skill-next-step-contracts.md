...
| Skill | End states | Valid recommendation |
| --- | --- | --- |
| `idea-scope-brief` | Business discovery lane missing, business discovery enabled, project type unclear, divergent paths detected | Pack missing -> `npx skillpacks install business-discovery`. Pack enabled -> `customer-discovery` using `research/idea-brief.md`. Project unclear -> `pack recommend`. Divergent paths -> `/fork-idea-branch`. |
