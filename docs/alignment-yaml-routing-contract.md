...
Skills must not use a `review` alignment page as a downstream command handoff. Do not include `Recommended next skill`, `Recommended next command`, `$exec`, `/exec`, or equivalent execution-loop routing inside the review page before final compiled YAML approval. The exception is the compiled YAML's top-level `command` field: it is required continuation metadata for the producing skill or parent orchestrator, not downstream routing.

Exception: self-advancing Pattern A research orchestrators may include `agent_routing` metadata in the compiled YAML and may end the **terminal message outside the page** with `## Next Work` and `## Invoke With YAML`. `## Next Work` owns the review/compile/paste instruction; `## Invoke With YAML` only names the same parent orchestrator command to invoke with the compiled YAML. This is loop continuation guidance, not downstream routing, and the alignment page itself remains a review artifact rather than a downstream command handoff. The compiled YAML must include a top-level `command` field with the same literal parent-orchestrator command shown under `## Invoke With YAML`; when `agent_routing.command` is present, root `command` and `agent_routing.command` must match exactly. The command value must name the parent orchestrator, never a child framework path command; the parent still interprets the YAML and filesystem state before writing artifacts or loading framework subskills inline.
