...
| --- | --- | --- | --- |
| `packs/base/claude/delegate/SKILL.md` | `hybrid` mode | Exits non-zero with `mode-mismatch:` naming resolved mode; packet untouched | § "Mode requirement" + § "Process" step 1 — `requires mode hybrid` by design |
| `packs/base/claude/delegate/SKILL.md` | `hybrid`; `codex` binary on PATH | Start marker never prints; enters pre-start-failure branch | § "Process" step 6 pre-start-failure branch: offer inline Claude execution or keep packet `approved`; `--inline-fallback` auto-selects inline |
| `packs/base/claude/delegate/SKILL.md` | `hybrid`; Codex exec completes cleanly | Non-zero exit or timeout after start marker → transport ambiguous | § "Process" step 6 ambiguous branch: `mark-uncertain` + inspect/discard/continue prompt; never blind-retries |
| `packs/base/claude/handoff/SKILL.md` | `--target=codex`: mode ≠ `codex-only` | Step 5.1 aborts with `mode-mismatch:` — Claude is not the planner in `codex-only` | § "Process" step 5.1 — `requires mode claude-only or hybrid` |
| `packs/base/claude/handoff/SKILL.md` | `--target=codex`: `jq` on PATH for pretty-print (step 5.5) | `scripts/approved-plan.sh draft` dies first via `require_jq_write` with the install-hint error; packet is never drafted | § "Process" step 5 preamble — `jq` declared a hard dependency; failure text cited |
| `packs/base/codex/exec/SKILL.md` | `--execute-approved`: mode ≠ `claude-only` | `scripts/approved-plan.sh check` prints `mode-mismatch` reason; skill stops with user error | § "Process" step 6c + § "Constraints" — `requires mode codex-only or hybrid` |
| `packs/base/codex/exec/SKILL.md` | `--execute-approved`: `jq` on PATH for consume write path | `scripts/approved-plan.sh consume` dies via `require_jq_write` with `ERROR: jq required for write operations. Install with: brew install jq …` | § "Process" step 6c + § "Constraints" — `jq` declared a hard dependency; failure text cited |
| `packs/base/claude/spec-interview/SKILL.md` | `any` mode resolved via `scripts/agent-mode.sh` | Mode lookup missing, unset, or non-zero degrades to inferred command text | § "Next-Step Routing" infers from invocation and task type |
| `packs/base/claude/roadmap/SKILL.md` | `any` mode resolved via `scripts/agent-mode.sh` | Mode lookup missing, unset, or non-zero degrades to inferred command text | § "Next-Step Routing" infers from invocation and task type |
| `packs/base/claude/plan-phase/SKILL.md` | `any` mode resolved via `scripts/agent-mode.sh` | Mode lookup missing, unset, or non-zero degrades to inferred command text | § "Next-Step Routing" infers from invocation and task type |
| `packs/base/claude/exec/SKILL.md` | `any` mode resolved via `scripts/agent-mode.sh` | Mode lookup missing, unset, or non-zero degrades to inferred command text | § "Next-Step Routing" infers from invocation and task type |
| `packs/base/claude/ship/SKILL.md` | `any` mode resolved via `scripts/agent-mode.sh` | Mode lookup missing, unset, or non-zero degrades to inferred command text | § "Next-Step Routing" infers from invocation and task type |
| `packs/base/claude/ship-end/SKILL.md` | `any` mode resolved via `scripts/agent-mode.sh` | Mode lookup missing, unset, or non-zero degrades to inferred command text | § "Next-Step Routing" infers from invocation and task type |
| `packs/base/codex/spec-interview/SKILL.md` | `any` mode resolved via `scripts/agent-mode.sh` | Mode lookup missing, unset, or non-zero degrades to inferred command text | § "Next-Step Routing" infers from invocation and task type |
| `packs/base/codex/roadmap/SKILL.md` | `any` mode resolved via `scripts/agent-mode.sh` | Mode lookup missing, unset, or non-zero degrades to inferred command text | § "Next-Step Routing" infers from invocation and task type |
| `packs/base/codex/plan-phase/SKILL.md` | `any` mode resolved via `scripts/agent-mode.sh` | Mode lookup missing, unset, or non-zero degrades to inferred command text | § "Next-Step Routing" infers from invocation and task type |
| `packs/base/codex/exec/SKILL.md` | `any` mode resolved via `scripts/agent-mode.sh` (Next-Step Routing block, distinct from `--execute-approved` rows above) | Mode lookup missing, unset, or non-zero degrades to inferred command text | § "Next-Step Routing" infers from invocation and task type |
| `packs/base/codex/ship/SKILL.md` | `any` mode resolved via `scripts/agent-mode.sh` | Mode lookup missing, unset, or non-zero degrades to inferred command text | § "Next-Step Routing" infers from invocation and task type |
| `packs/base/codex/ship-end/SKILL.md` | `any` mode resolved via `scripts/agent-mode.sh` | Mode lookup missing, unset, or non-zero degrades to inferred command text | § "Next-Step Routing" infers from invocation and task type |
...
- ~~`packs/base/claude/handoff/SKILL.md` `--target=codex`~~ — Resolved: step 5 preamble now declares `jq` required and cites the `require_jq_write` error that fires at draft time (before the 5.5 pretty-print is reached).
- ~~`packs/base/codex/exec/SKILL.md` `--execute-approved`~~ — Resolved: step 6c now names the exact user-facing failure reason when `jq` is missing; § "Constraints" unchanged.

## Pack emphasis

This section tags every base skill and every pack with a **primary CLI role** — `Claude-orchestration` (framing, interviews, strategy, research synthesis, tradeoff surfacing), `Codex-execution` (implementation, reconciliation, validation, repo mutation, shipping), or `Both` (genuinely spans). Role tagging is additive and advisory: parity-mirror skills still exist under both `packs/base/claude/` and `packs/base/codex/`, and the tag names the *intended* emphasis, not a restriction. Kanban pack variants inherit from their base pack unless noted. This is the one authoritative table — pack-level docs reference it rather than duplicate.
...
| `init-agentic-skills` | Both | Initializes base Claude/Codex managed skill installs; pack setup remains project-local through `init-agentic-skills` / `scripts/pack.sh` |
...
| `business-research` | Claude-orchestration | ICP, customer/market research, value proposition, positioning, lean canvas |
...
| `business-app` | Claude-orchestration | Compatibility alias for `business-research`, `customer-lifecycle`, `business-growth`, and `business-ops`; `business-discovery` remains an accepted legacy alias for `business-research` |
