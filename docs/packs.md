...
Use `npx skillpacks uninstall <name>` to remove selected project-local packs or skills. It ends with the exact install command needed to restore those targets. Use `npx skillpacks uninstall --all` to preview and, after an interactive confirmation, remove all project-local skillpacks-managed roots and install settings. Unmanaged roots and unrelated config fields are preserved, and the final output is one copy-paste command line that restores the previous selection. Non-interactive `--all` runs cancel safely. The older `remove <name>` command remains available.
