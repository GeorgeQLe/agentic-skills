...
| `uninstall <name...>` | Node-owned | Manifest plus lifecycle helpers | No | No | Removes selected project-local packs or skills and ends with the exact command needed to reinstall the selection. |
| `uninstall --all` | Node-owned | Managed marker ownership reader plus project config writer | No | No | Previews all project-local skillpacks-managed installs and settings, requires interactive confirmation, preserves unmanaged roots and unrelated config, and ends with one command line that restores the prior selection. |
...
| `alignment pages open <alignment/page.html\|briefing-slides/page.html> [--browser <browser>]` | Node-owned wrapper | Packaged `scripts/open-html-page.mjs` | No | No | Opens or focuses an active rendered alignment page or briefing-slide deck with best-effort platform handling. Browser values: `auto`, `brave`, `chrome`, `safari`, `edge`, or `default`. |
...
| `briefing slides audit` | Node-owned wrapper | Packaged `scripts/audit-briefing-slides.mjs` | No | No | Audits active rendered `briefing-slides/*.html` decks with `--root <cwd>`. |
