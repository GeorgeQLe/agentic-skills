...
`jq` is required for source-checkout `scripts/pack.sh` write operations (install, remove, refresh, set-mode, set-update-mode, set-bip, set-bip-platforms). Node-owned `npx skillpacks` project-config commands do not require `jq`, except `install-deck` still uses shell materialization.
