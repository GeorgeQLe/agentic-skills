...
| 4 | Bench via vitest (`bench:quick`) | minutes | ~$1/exec |
...
2. Bench (only if verify passes): `pnpm bench --skill <SKILL> --agent both --runs 3 --chunk-size 3 --pause 0` — 3 iterations per agent, ~$1/exec, skill timeout per run.
...
- **Budget:** ~$1/exec for design-system variant
