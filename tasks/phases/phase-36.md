...
### Green
- [x] Step 36.9: Run tests, representative benchmarks, and phase review.
  - Classification: automated
  ...
  - Review: Ran the Phase 36 validation gate, representative live Codex benchmarks, and standard skill audits. Validation found one over-specific `investigate` setup assertion that forced the literal `$run` route; the fixture asks for a diagnostic report with a concrete next command and no source edits, so the setup now requires an actionable next-command handoff without forcing `$run`. No other cleanup was required.
  - Ship manifest:
    - User goal: Execute Phase 36 Step 36.9 from `$run` by running final tests, representative quality-scored Codex benchmarks, standard audits, phase review, and phase closure.
    - Changed files: `tests/layer4/setups/tier1-workflows.setup.ts`, `tasks/todo.md`, `tasks/roadmap.md`, `tasks/history.md`, `tasks/phases/phase-36.md`.
    - Per-file purpose: `tier1-workflows.setup.ts` fixes the `investigate` benchmark hard assertion to match the diagnostic-only fixture contract; `tasks/todo.md` records Step 36.9 evidence, acceptance criteria, and phase completion; `tasks/roadmap.md` marks Phase 36 complete; `tasks/history.md` records the shipped outcome; `tasks/phases/phase-36.md` archives the completed phase.
    - User-goal mapping: The validation commands prove benchmark output-quality reporting and setup coverage are working end to end, while the setup fix removes a false negative discovered by the required representative `investigate` live benchmark.
    - Tests run: `pnpm --dir tests test:layer1 -- bench-quality bench-report bench-setups runner coverage-status` passed with 10 files and 1245 tests before and after the setup fix; `pnpm --dir tests bench:coverage` passed with 143 skills before and after the setup fix; `pnpm --dir tests bench --list-skills` passed and listed all coverage rows; `pnpm --dir tests bench --skill run --agent codex --runs 1 --chunk-size 1 --pause 0` passed with session `run-codex-202607cd`, hard pass rate 100.0%, quality score 100.0%; `pnpm --dir tests bench --skill investigate --agent codex --runs 1 --chunk-size 1 --pause 0` initially failed because of the over-specific `$run` assertion, then passed after the fix with session `investigate-codex-a0dd4352`, hard pass rate 100.0%, quality score 100.0%; `pnpm --dir tests bench --skill design-system --agent codex --runs 1 --chunk-size 1 --pause 0` passed with session `design-system-codex-c65b7386`, hard pass rate 100.0%, quality score 90.9%; `pnpm --dir tests bench --skill run-kanban --agent codex --runs 1 --chunk-size 1 --pause 0` passed with session `run-kanban-codex-776ecf18`, hard pass rate 100.0%, quality score 97.5%; `./scripts/skill-deps.sh --broken` passed; `./scripts/skill-versions.sh --missing` passed with all 316 skills versioned; `./scripts/skill-next-step-routing.sh --missing` passed with all 229 mutation-capable skills routed; `git diff --check` passed.
    - Skipped tests: Full multi-run benchmark matrices were not run because Phase 36 requires representative one-run Codex benchmarks, and earlier deterministic tests cover the scoring/report contracts without spending additional live-agent budget. Skills Showcase refresh was not run because this step did not change tracked `SKILL.md` or `PACK.md` files.
    - Adversarial review: Diff-aware self-review checked that the setup fix aligns the hard assertion with the prompt, keeps quality scoring for concrete next-command handoff, preserves evidence/root-cause/fabrication checks, does not change prompts, budgets, schemas, dependencies, lockfiles, or GitHub Actions, and does not weaken other Tier 1 route assertions.
    - Residual risk: The live benchmark evidence is one run per representative skill, so it proves output-quality reporting and rubric execution but not statistical stability. The report language already separates Wilson confidence from the optional quality score to avoid overstatement.
    - Rollback note: Revert the Phase 36 Step 36.9 commit to restore the previous `investigate` route assertion and task-document state.
