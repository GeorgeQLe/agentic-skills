...
- [x] Step 32.5: Update skill mutation contracts to maintain the website after skill changes.
  ...
  - Verification plan:
    ...
    - Run `git diff --check`.
- [x] Step 32.6: Validate and record the phase.
...
- [x] Run `node scripts/generate-skills-showcase-data.mjs`.
- [x] Run `node scripts/generate-skills-showcase-github-data.mjs`.
...
- [x] Run focused tests for any added generator or validator coverage.
...
#### 2026-05-07 - Step 32.6 Ship Manifest

- **User goal:** Execute `$run` for the next incomplete Phase 32 step by running the full validation gate, recording the phase result, and preparing the next phase.
- **Changed files:** `tasks/todo.md`, `tasks/history.md`, `tasks/roadmap.md`, `tasks/phases/phase-32.md`.
- **Per-file purpose:** `tasks/todo.md` records the completed Phase 32 status before archival and then becomes the active Phase 33 plan; `tasks/history.md` records the Phase 32 validation result; `tasks/roadmap.md` marks Phase 32 complete and Phase 33 active; `tasks/phases/phase-32.md` preserves the completed Phase 32 record.
- **User-goal mapping:** Step 32.6 required full validation and archive-ready phase status. All Phase 32 acceptance criteria are checked, validation passed, and the next phase has a self-contained execution plan.
- **Tests run:** `node scripts/generate-skills-showcase-data.mjs` passed; `node scripts/generate-skills-showcase-github-data.mjs` passed; `node --check docs/skills-showcase/app.js` passed; `bash -n scripts/validate-skills-showcase-data.sh` passed; `scripts/validate-skills-showcase-data.sh` passed; static route file presence check passed for all six routes plus shared CSS/JS; targeted generated-data scans passed; `./scripts/skill-deps.sh --broken` passed; `./scripts/skill-versions.sh --missing` passed; `./scripts/skill-next-step-routing.sh --missing` passed; `./scripts/skill-pack-routing-audit.sh` passed; `pnpm --dir tests test` passed with 1,177 layer1 tests; `git diff --check` passed.
- **Skipped tests:** Browser screenshot/responsive UI validation was not run for Phase 32 because the phase foundation is static shell, data, and workflow contracts. Phase 33 explicitly owns the richer interactive UI, generated catalog rendering, responsive behavior, and browser verification.
- **Adversarial review:** Validation checked that the phase did not add a database, video/Remotion implementation, runtime API, GitHub Actions workflow, unnecessary root dependency, secrets, live LexCorp metrics, or visitor analytics. Generated proof data states those boundaries explicitly.
- **Residual risk:** `generatedAt` is a stable placeholder while `sourceFingerprint` is the true freshness signal; Phase 33 should avoid presenting `generatedAt` as a wall-clock timestamp. Visual polish and responsive behavior still need browser verification in Phase 33.
- **Rollback note:** Revert the Step 32.6 commit to restore Phase 32 as active and remove the Phase 33 active todo handoff.
- **Next command:** `$run`
...
Start Phase 33 Step 33.1 by wiring generated catalog/search interactions to the static showcase routes.
