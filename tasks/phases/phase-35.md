...
- [x] Step 35.9: Write regression tests and final validation evidence for the coverage contract.
  - Classification: automated
...
- Deviations from plan: The review-only subagent lane was not used because active Codex subagent instructions only permit subagents when explicitly requested by the user; local adversarial review and expanded executable regression coverage were used instead.
- Tech debt / follow-ups: Claude parity for custom benchmark setups remains intentionally deferred by the phase scope. Current blocked rows document unsafe or external-fixture follow-ups with concrete next commands.
- Ready for next phase: Yes. Phase 35 acceptance criteria and validation gates are complete.

### Step 35.9 Review — Coverage Contract Regression Tests and Final Evidence

- Added `tests/layer1/bench-coverage.test.ts` to isolate final coverage-contract regressions for complete matrix validation, missing rows, duplicate rows, unknown rows, missing custom setup paths, missing blocked reasons, missing blocked next commands, custom/generic/blocked resolution, generic fallback retention, list/status CLI output, zero-run custom status output, and blocked preflight behavior.
- Extended `tests/layer1/runner.test.ts` with exact resume-config matching coverage and non-zero-only rate-limit infrastructure-block classification coverage.
- Validation found stale generated Skills Showcase proof data after repository validation-script metadata changed; refreshed `docs/skills-showcase/assets/github-proof-data.js` and confirmed the freshness check passes.
- Ran representative one-run Codex benchmarks for `run`, `plan-phase`, `benchmark-test-skill`, and pack skill `youtube-video-audit`; all passed with 100% pass rate.
- Execution profile note: the phase requested a review-only lane after Step 35.6, but active Codex subagent instructions only permit subagents when explicitly requested by the user. I used focused executable regression coverage, live benchmark evidence, and diff-aware local adversarial review instead.

**Validation:**
- `pnpm --dir tests exec vitest run --project layer1 layer1/bench-coverage.test.ts layer1/bench-setups.test.ts layer1/runner.test.ts` — passed, 3 files / 43 tests.
- `pnpm --dir tests bench:coverage` — passed, `Benchmark coverage matrix valid (143 skills).`
- `pnpm --dir tests bench --list-skills` — passed and printed custom/blocked coverage status for all 143 repository skills.
- `pnpm --dir tests test:layer1` — passed, 9 files / 1229 tests.
- `pnpm --dir tests bench --skill run --agent codex --runs 1 --chunk-size 1 --pause 0` — passed, report `tests/benchmarks/runs/run-codex-71aeaa8a/report.md`, pass rate 100%.
- `pnpm --dir tests bench --skill plan-phase --agent codex --runs 1 --chunk-size 1 --pause 0` — passed, report `tests/benchmarks/runs/plan-phase-codex-0ea4e69c/report.md`, pass rate 100%.
- `pnpm --dir tests bench --skill benchmark-test-skill --agent codex --runs 1 --chunk-size 1 --pause 0` — passed, report `tests/benchmarks/runs/benchmark-test-skill-codex-430972d6/report.md`, pass rate 100%.
- `pnpm --dir tests bench --skill youtube-video-audit --agent codex --runs 1 --chunk-size 1 --pause 0` — passed, report `tests/benchmarks/runs/youtube-video-audit-codex-288097bc/report.md`, pass rate 100%.
- `./scripts/skill-deps.sh --broken` — passed, no broken references found.
- `./scripts/skill-versions.sh --missing` — passed, all 316 skills have a version field.
- `./scripts/skill-next-step-routing.sh --missing` — passed, all 229 mutation-capable skills have next-step routing.
- `./scripts/skill-pack-routing-audit.sh` — passed, no cross-pack recommendation gaps found.
- `scripts/validate-skills-showcase-data.sh` — initially failed with stale `docs/skills-showcase/assets/github-proof-data.js`, then passed after regeneration; reran after phase-archive/task-doc updates and passed again.
- `git diff --check` — passed before and after task-doc updates.

**Quality Gate Manifest:**
- User goal: execute Step 35.9 for repository-wide custom benchmark coverage and close Phase 35 with final regression evidence.
- Changed files: `tests/layer1/bench-coverage.test.ts`, `tests/layer1/runner.test.ts`, `docs/skills-showcase/assets/github-proof-data.js`, `tasks/todo.md`, `tasks/history.md`, `tasks/roadmap.md`, `tasks/phases/phase-35.md`.
- Per-file purpose: add final coverage-contract regression tests; extend runner regression coverage for resume and infrastructure-block behavior; refresh generated proof-data freshness fingerprint; mark Step 35.9 and Phase 35 complete; archive the completed phase; record history.
- User-goal mapping: the new tests prove the required benchmark coverage failure modes, reporting statuses, fallback behavior, resume behavior, and infrastructure-block classification; the docs record exact evidence and phase closeout.
- Tests run: listed above.
- Skipped tests: layer2/layer3 Vitest suites were not run because the changed executable surface is benchmark harness and layer1 contract coverage. Claude live benchmarks were not run because the phase is Codex-first and representative live evidence was explicitly Codex.
- Adversarial review: diff-aware self-review checked that tests assert independent reason/next-command failures, synthetic generic fallback remains runnable, blocked coverage exits before agent execution, generated asset change is fingerprint-only freshness output, no GitHub Actions files are touched, and no benchmark behavior changed beyond test coverage.
- Residual risk: blocked coverage rows still require future deterministic fixture design before they can become custom; those rows carry explicit next commands. Live benchmark pass rates are based on one run per representative setup, which is useful smoke evidence but not statistical confidence.
- Rollback note: revert the Step 35.9 commit to remove the added regression tests, generated proof-data refresh, and phase-close task docs.
