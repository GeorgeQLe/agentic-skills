...
**Goal:** Use persisted benchmark data as the source of truth for relevant gSkillPacks demo sections, showing representative prompts and outputs instead of hand-wavy curated examples.

**Plan:**
- [x] Inspect current showcase generator/app demo rendering and benchmark artifact shape.
- [x] Extend generated skill data with sanitized prompt/output demo fields sourced from benchmark run JSON.
- [x] Update the brand site demo UI to render benchmark-backed prompt/output evidence where available.
- [x] Add focused tests for generator data shape and fallback behavior.
- [x] Regenerate showcase assets, run validation, and record review results.

**Review:** Complete. Catalog benchmark panels now render prompt/output demo excerpts from persisted benchmark run artifacts when available. Generated demo excerpts remove local temp paths, link back to the run JSON artifact, and preserve the existing benchmark metrics panel. Validation passed with `node scripts/generate-skills-showcase-data.mjs`, focused layer1 benchmark-demo coverage, focused showcase catalog tests, app typecheck, `scripts/validate-skills-showcase-data.sh`, and `git diff --check`.
...
**Verify:** Passed. Layer1 passed in 9.2s across 1,303 tests. Layer2 had no matching target-specific tests for `benchmark-test-skill` and was skipped.
...
## Current Documentation Update: gSkillPacks Brand

**Goal:** Record `gskillpacks.com` as the production domain and align public site/docs language around the **gSkillPacks** brand while keeping `agentic-skills` as the underlying open-source library name.

**Plan:**
- [x] Update site metadata, header, homepage, footer, and app README to use **gSkillPacks** and `gskillpacks.com`.
- [x] Update product/UI/newsletter specs with the brand/domain decision and skill packs layout vocabulary.
- [x] Update root design guidance so future UI work preserves gSkillPacks naming, packs language, routes, coordinates, nodes, pack maps, and proof surfaces.
...
**On Completion**
- Deviations from plan: DB uses uuid for id instead of serial; 74 tests total (vs 73 noted in earlier summaries due to one test added during validation). No other deviations.
- Tech debt / follow-ups: Vercel deploy not yet configured (manual). Admin does not support edit/delete/status-management per non-goals. Newsletter sending is out of scope.
