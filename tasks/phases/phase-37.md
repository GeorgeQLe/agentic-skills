...
- [x] Step 37.6: Retire or mark the old static-site surface as superseded only after the app routes and generated assets are validated
...
- [x] Step 37.8: Run local app validation, generated-data validation, and whitespace checks; perform only concrete cleanup found by validation
...
**Acceptance Criteria:**
- [x] A minimal Next.js Skills Showcase app exists and can render the migrated public routes.
- [x] Existing showcase content, styling intent, catalog, workflow, pack, inspect, and follow surfaces are preserved.
- [x] Generated skill/proof data still loads from committed assets and freshness validation remains available.
- [x] Old static-site deployment assumptions are replaced or clearly superseded by the Next.js app deployment contract.
- [x] Newsletter capture remains non-persistent until Phase 38.
- [x] Local app validation, generated-data validation, and whitespace checks pass.
- [x] No GitHub Actions are created, modified, or recommended.
- [x] All phase tests pass.
- [x] No regressions in previous phase tests.

**On Completion**
- Deviations from plan: None. All 8 steps executed as planned. Step 37.8 validation found stale generated data which was regenerated and committed.
- Tech debt / follow-ups: `docs/skills-showcase/assets/` dual-write kept for safety until first Vercel deploy. Follow-up todo tracks benchmark-results surface (Phase 39). Two non-blocking manual tasks remain: newsletter provider config and Vercel project setup.
- Ready for next phase: yes

### Step 37.8 Result

- All 5 validation commands passed: tests (54/54), typecheck (clean), build (6 static routes), data validation (fresh after regeneration), whitespace (clean).
- Layer1 regression check passed: 1256/1256 tests across 10 files.
- Regenerated stale generated data assets (319 skills, 17 packs; 4 proof artifacts, 5 validation scripts) — committed as `chore: regenerate Skills Showcase data assets`.
- Phase 37 milestone fully met. Phase complete.

## Review

- 2026-05-12 — Completed Step 37.8: local app validation and whitespace checks. All 5 validation commands passed. Regenerated stale generated data assets. Layer1 regression check passed (1256/1256). Phase 37 milestone fully met — all 9 acceptance criteria checked off.
...
- Deleted 8 static site files: `index.html`, `app.js`, `styles.css`, and 5 route `index.html` files under `catalog/`, `workflows/`, `packs/`, `inspect/`, `follow/`.
- Kept `docs/skills-showcase/assets/` with dual-write intact (safest until first Vercel deploy).
- Updated `docs/skills-reference.md`: removed "retained for compatibility until Step 37.6" language, marked Next.js app as sole showcase surface, noted `docs/skills-showcase/assets/` kept for dual-write.
