...
- Decision applied per task: hard rename, no legacy fallback; coordinated mechanical sync without per-consumer version bumps (only `research/<file>.md` strings change; "product concept" prose preserved). value-prop-canvas was a pure body-string rename (3 occurrences/file), so no showcase metadata changed for it.
- Regenerated Skills Showcase assets (`skills-data.js` ×2, `github-proof-data.js` ×2, `docs/benchmark-results-matrix.md`) — these picked up already-committed producer changes (`idea-scope-brief` v0.6→v0.8, `icp` argument-hint) that had left the generated data stale; validator reports fresh.
- Captured `/exec` prompt-history for the three rename steps under `prompts/exec/`.
- Verification: active value-prop-canvas/icp/competitive-analysis/lean-canvas SKILL.md files have zero `concept-brief` references; `git diff --check` clean; remaining `concept-brief` mentions are all out-of-scope later phases (prototype, spec-interview, research-roadmap, afps-status, tier23 test) or intentional producer migration notes / historical logs.
- Also shipped unrelated Next.js build drift (`apps/skills-showcase/next-env.d.ts`: `.next/types` → `.next/dev/types`) as a separate chore commit so no tracked changes are left behind.

## 2026-05-30 — Code Review test-infra #4–#6: disposable-repo + bench-persistence

Cleared the last three Code Review items (all test infra). #4: `cleanupRepo` in `tests/layer4/helpers/disposable-repo.ts` deleted GitHub repos via shell `execSync` with an unvalidated slug (could target `unknown/<name>`); added `isSafeBenchRepoSlug()` guard (bench-prefix + non-`unknown` owner) checked before delete and switched to `execFileSync` (no shell). #5: `cleanup()` leaked every `mkdtempSync` clone; threaded `workDir` into cleanup to `rmSync` it, and `rmSync` the orphaned `sync-upstream-` clone in `git-fixture-sync.setup.ts`. #6: `findResumeableSession` sorted session dirs by random UUID suffix; extracted pure `pickResumeableManifest()` sorting by `updatedAt`/`createdAt`. Verified (Node 25): new `tests/layer1/code-review-test-infra-fixes.test.ts` 8/8 pass in isolation; full layer1 1798 tests / 1796 pass / 2 fail — the 2 are in `output-paths.test.ts` (`skill-dev` output-path conflict), pre-existing and unrelated (verified failing at parent commit `ebbe3267` with none of my edits; imports none of the touched modules; new test passes 8/8 standalone). `git diff --check` clean. (`tsc --noEmit` is not a usable gate here — tests/tsconfig omits `@types/node` so it errors project-wide on clean master.) Key files: `tests/layer4/helpers/disposable-repo.ts`, `tests/layer4/setups/git-fixture-sync.setup.ts`, `tests/harness/bench-persistence.ts`.
...
## 2026-05-17 — roadmap benchmark fresh rerun

- Reran `$benchmark-test-skill roadmap` after the benchmark-results matrix assertion fix.
- Verified `roadmap` custom coverage and passed `pnpm verify --skill roadmap` with layer1 PASS in 3.6s and layer2 SKIP.
...
## 2026-05-30 — Fix 94 pre-existing layer1 test failures

- Cleared the entire pre-existing layer1 backlog: `pnpm --dir tests exec vitest run --project layer1` now reports **1760 passed / 0 failed** (was 94 failed). Root cause across most suites: stale tests/fixtures/generators never updated after the 53-global-skill → 22-pack split (`8f386368`) and the `run`→`exec` / `install-agentic-skills`→`init-agentic-skills` renames.
- Test/fixture/generator fixes: `output-paths.test.ts` (count distinct skills per path, skip directory/glob outputs, allowlist the shared `tests/harness/bench-coverage.ts` registry and monorepo placeholder example paths — 77→0); `codex-interview-cadence.test.ts` (repointed 7 moved skills to `packs/product-design/codex/*` and `packs/agent-work-admin/codex/roadmap`); `init-agentic-skills-contract.test.ts` (`v0.4`→`v0.5`); `bench-coverage.ts` (added codex-only `afps-status`); `pack-workflows.setup.ts` (added the missing `product-line` pack-workflow setup); `generate-skills-showcase-data.mjs` (`WORKFLOW_SKILL_MAP` `run`→`exec`, regenerated both showcase asset copies + benchmark matrix); `benchmark-results-matrix.test.ts` (corrected diagnosis — it was a stale `run-codex-*`→`exec-codex-*` run-directory regex, not the generator map).
- One genuine skill gap (not a stale test): `youtube-audit` (claude+codex) was the only skill of 48 in the creator/youtube/remotion packs missing the `## Approved Artifact Handoff` and `## Intent-Aware Routing` contract sections its peers carry. With user approval, added the sections (mirroring `youtube-channel-audit`, using "next-step routing" phrasing since youtube-audit has no `## Next-Skill Routing` section), archived v0.1, bumped to v0.2, and updated both CHANGELOGs.
- Validation: full layer1 green (1760/1760); `bash scripts/skill-versions.sh --missing` → all 390 skills versioned; `git diff --check` clean; only intended files changed (tests, harness, generator, generated assets, and the two approved youtube-audit skills).

## 2026-05-30 — Harden newsletter admin session auth (Code Review High #1)

- Resolved the High-severity `/expert-review` (2026-05-29) finding on `/admin/newsletter` auth. Stateless, dependency-free design using only Node's `crypto` — no DB, no new packages, no new env vars.
- New `apps/skills-showcase/src/trpc/session.ts`: `safeSecretEqual` (SHA-256 → `timingSafeEqual`, length-safe), `createSessionToken` (`v1.${expiresAt}.${nonce}.${sig}` HMAC-SHA256, 24h TTL, per-login 16-byte nonce for rotation), `verifySessionToken` (validates shape/expiry/signature, constant-time, never throws), exported `SESSION_TTL_MS` + `SESSION_COOKIE_NAME`.
- `adminLogin` now constant-time-compares the secret and sets the **signed token** as the cookie (raw secret no longer on the wire); `Max-Age` derived from `SESSION_TTL_MS/1000`. `protectedProcedure` verifies via `verifySessionToken`. Four concerns closed: secret off-wire, 24h expiry, nonce rotation, `timingSafeEqual` both paths.
- Quality gate: adversarial review caught a self-introduced DoS — a 64-char non-hex signature passed the char-length guard but `Buffer.from(...,'hex')` truncated the buffer, throwing a 500 in `protectedProcedure`. Fixed to decode-then-compare byte lengths; added a non-hex-signature regression test.
- Validation: `pnpm --dir apps/skills-showcase test` → **110 passed** (new `session.test.ts` + updated `newsletter.test.ts`); `pnpm --dir apps/skills-showcase typecheck` → clean. Required Node 25 (repo default 20.17 too old for vite8/vitest4) + a manually-placed rolldown native binding (pre-existing test-infra, backlog #3–#6).
- Vercel auto-deploys the live app on push to master. Post-deploy manual `/admin/newsletter` auth-gate check (browser, human-only) remains per `tasks/deploy.md` launch checks.
