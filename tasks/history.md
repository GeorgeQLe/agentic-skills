...
## 2026-05-11 — UI variant evaluation route and run benchmark ship
...
- Moved `ui-consolidate` from blocked benchmark coverage to a custom deterministic setup and recorded the successful both-agent `run` benchmark evidence.
- Refreshed Skills Showcase generated assets after tracked skill metadata changed.
- Ship manifest:
  - User goal: Ship already-finished work in the tree, including UI variant workflow contracts, `ui-consolidate` coverage, `run` benchmark evidence, and the Phase 36 `investigate` benchmark quality false-negative fix.
  - Changed files: `benchmark/test-run-2026-05-11.md`, `docs/canonical-workflow-report.md`, `docs/skill-next-step-contracts.md`, `docs/skills-reference.md`, `docs/skills-showcase/assets/github-proof-data.js`, `docs/skills-showcase/assets/skills-data.js`, `global/claude/skills/SKILL.md`, `global/claude/uat/SKILL.md`, `global/claude/ui-consolidate/SKILL.md`, `global/claude/ux-variation/SKILL.md`, `global/codex/design-system/SKILL.md`, `global/codex/skills/SKILL.md`, `global/codex/uat/SKILL.md`, `global/codex/ui-consolidate/SKILL.md`, `global/codex/ui-consolidate/agents/openai.yaml`, `global/codex/ui-interview/SKILL.md`, `global/codex/ux-variation/SKILL.md`, `tasks/history.md`, `tasks/lessons.md`, `tests/harness/bench-coverage.ts`, `tests/layer1/bench-setups.test.ts`, `tests/layer4/setups/tier1-workflows.setup.ts`, `tests/layer4/setups/tier23-global-workflows.setup.ts`.
  ...
  - User-goal mapping: The shipped boundary packages the existing completed benchmark work, preserves the user-corrected lesson that consolidation can be premature, and makes the route enforceable through skill contracts and benchmark coverage.
  - Tests run: `scripts/validate-skills-showcase-data.sh` passed; `pnpm --dir tests test:layer1 -- bench-setups bench-report` passed with 10 files and 1245 tests; `pnpm --dir tests bench:coverage` passed with 143 skills; `pnpm --dir tests bench --list-skills` listed `ui-consolidate` as custom and `run` as custom; `/opt/homebrew/bin/bash scripts/skill-deps.sh --broken` passed; `/opt/homebrew/bin/bash scripts/skill-versions.sh --missing` passed with 317 versioned skills; `/opt/homebrew/bin/bash scripts/skill-next-step-routing.sh --missing` passed with 230 mutation-capable skills routed; targeted `rg` scans confirmed requirements-only, layout-mode, variant-evaluation, and consolidation routing language; `git diff --check` passed.
  - Skipped tests: Full `pnpm --dir tests test:layer1` was not rerun after the final docs-only/index additions because the focused layer1 suite already exercised benchmark registry, reporting, frontmatter, routing graph, and output path coverage for the changed surfaces. Live both-agent `run` benchmark evidence was already recorded in `benchmark/test-run-2026-05-11.md`.
  - Adversarial review: Diff-aware self-review checked that consolidation is gated on evaluation evidence, UAT remains human-run, Codex/Claude route syntax is platform-correct, `ui-consolidate` has version/frontmatter/routing coverage, generated assets are fresh, and the `investigate` quality rubric no longer overfits exact fixture wording.
