...
## 2026-07-03 — Skill fixes must target canonical source and go through publish + refresh

- A `/session-triage` scenario: the user extracts full session evidence to hand an agent running in `agentic-skills`, intending to patch the shared skill, publish it, and refresh installs. But agents frequently start from a different directory and patch only a local **installed copy** of the skill — a managed mirror (`.claude/skills`, `~/.claude/skills`, `~/.npm/**/skillpacks/…`) or a consuming repo's `.claude/skills`. That copy never propagates; the shared skill and every other install stay broken.
- Root cause (contract gap): `session-triage` treated all skill copies correctly as read-only *evidence* (step 1), but its fix recommendation (step 6) and handoff only said "name exact skill file(s)" and "route to `/targeted-skill-builder`." Nothing separated the evidence-source from the fix-target, forbade naming a mirror, or required the publish + refresh loop, so an implementing agent landed the edit on whatever copy was nearest.
- Rule: separate **evidence-source** (any copy is fair game to read) from **fix-target** (canonical only). A skill fix must target the canonical `packs/<pack>/{claude,codex}/<skill>/SKILL.md` source in the managing repo (resolve `<pack>` with `scripts/pack.sh which <skill>`; both variants when both exist), never a managed mirror or installed copy. It is not complete until published (`git add` edits → `npm run skillpacks:build` → `git add` + commit the regenerated manifest with the source) and refreshed (`scripts/pack.sh refresh` in-repo, or consumer `npx skillpacks refresh` + `/reload-skills` / fresh Codex session). When the agent's cwd is a different repo, route the fix back to the managing `agentic-skills` repo rather than editing the local copy; if the managing repo is not checked out locally, stop and say so.
- Fixed by adding a "Canonical Fix Target" subsection + publish/refresh loop to step 6, extending the Output (Recommended fix / Validation plan) and Constraints, and requiring the `/targeted-skill-builder` handoff to carry the canonical target path + publish/refresh loop, in both session-triage variants (v0.9, claude + codex).
- Follow-up (v0.10, same day): the user removed the `targeted-skill-builder` and `create-agentic-skill` implementer skills entirely (archived out of `packs/skill-dev`). `session-triage` no longer routes a fix to any builder skill. Its step 6 now classifies the defect as **managing-layer** (shared skill / convention page / workflow routing-process → emit a self-contained **YAML handoff payload** for an agent in the agentic-skills directory to implement + publish + refresh) or **invoking-directory** (rare → in-place patch report for the current dir, no managing-repo route). Payload schema authored in `docs/session-triage-handoff-contract.md`. All ~6 caller skills that routed to the archived builders (analyze-sessions, skill-interview, base/skills, prompt-history-backfill, benchmark-test-skill, benchmark-agent-review) were repointed to `session-triage` / `create-local-skill` / direct implementation. Rule: the implementer for a shared-skill fix is an agent running the YAML payload in the managing repo, not a dedicated builder skill.

## 2026-07-04 — Cleanup command output can be true but incomplete
