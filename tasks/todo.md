...
- Inspect the product-design pack routing surfaces before editing: `packs/product-design/PACK.md`, neighboring skill contracts such as `prototype`, `consolidate-prototypes`, and `design-inspirations`, route or alias metadata that enumerates product-design skills, and generator inputs that decide which convention bundles new skills receive.
- Create mirrored `packs/product-design/{codex,claude}/create-ui-experiment/` directories with `SKILL.md` and `CHANGELOG.md`. Start both active contracts at `version: v0.0`.
- Define `create-ui-experiment` as the owner of clickable UI experiment routes or project-native lightweight prototypes for one approved UI branch. The contract must include the exact test-covered language: `name: create-ui-experiment`, `version: v0.0`, "Own clickable UI experiment routes or project-native lightweight prototypes", "fake, fixture, local, or in-memory data", "first-value journey", and "progressive reveal".
- Keep the experiment scope pre-production: fake/fixture/local/in-memory data only, no durable database/auth/payments/analytics/deployment/admin/multi-tenant work unless a later post-experiment plan explicitly promotes it.
- Require the experiment to introduce first value and the primary task path before dense secondary controls, and require explicit review evidence before handoff into `$prototype`, `$uat --variant-evaluation`, or `$user-flow-map --prototype-build-plan` (Claude mirrors use `/prototype`, `/uat --variant-evaluation`, and `/user-flow-map --prototype-build-plan`).
- Register the new skill in `packs/product-design/PACK.md` with the test-covered summary "`create-ui-experiment`: Own clickable UI experiment routes"; update any routing docs or alias metadata discovered in the first step.
- Generate required convention bundles for the new skill (`DESIGN-TREE-LOOP.md`, and `ALIGNMENT-PAGE.md` / `INTERROGATION-PAGE.md` if required by frontmatter), then stage source skill edits before regenerating `packages/skillpacks/dist/skillpacks-manifest.json` and Skills Showcase assets.
- Verification target: run `pnpm --dir tests exec vitest run --project layer1 layer1/product-design-flow-tree.test.ts` and confirm all 14 tests pass; run generator checks for any bundles added, `scripts/skill-archive-audit.sh --strict`, `apps/skills-showcase/scripts/validate-skills-showcase-data.sh`, `npm run skillpacks:verify`, `node scripts/audit-task-docs.mjs`, and `git diff --check`.
