...
- Workflow skills now front-load missing and stale documentation work into `tasks/todo.md` instead of only reporting read-only status
...
Use `/workflow` or `$workflow` to refresh `tasks/todo.md` with priority documentation items, or start a new spec and roadmap cycle if new work is being introduced.
