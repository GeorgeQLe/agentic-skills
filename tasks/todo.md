...
- [ ] Add review-gate precedence clause to the **Pre-approval stop** — `docs/alignment-page-convention.md` (~line 123) — when a chunked assemble stop's next action is the binding review gate (page in `review`), the Pre-approval stop governs over any chunk Progress Handoff repeat-command framing; present exactly one YAML (the page's compiled response YAML). Write generically for all chunked design skills.
- [ ] Add assemble-ready carve-out to **§5 Progress Handoff Block** — `docs/design-tree-loop-convention.md` (~lines 454–481) — at the assemble-ready stop where the deliverable's alignment page is in `review`, the block must not foreground "repeat this command" or a `## Invoke With YAML` payload; it foregrounds "Review required" and presents only the compiled response YAML. Reserve repeat-command framing for setup + per-unit stops.
- [ ] Mirror the carve-out in the codex skill — `packs/product-design/codex/ux-variations/SKILL.md` (§0c Required Progress Handoff Block lines ~82,89–92; step 8 line ~285) — assemble-ready leads with the HTML review path + single compiled response YAML; no same-command rerun line, no second YAML.
- [ ] Mirror the same carve-out in the claude variant — `packs/product-design/claude/ux-variations/SKILL.md` — identical change with `/ux-variations` syntax.
- [ ] Version bump v0.32 → v0.33 — run `scripts/skill-archive.sh packs/product-design/codex/ux-variations` and `scripts/skill-archive.sh packs/product-design/claude/ux-variations`, bump `version:` frontmatter in both, add `CHANGELOG.md` entries describing the assemble-stop review-gate carve-out — behavioral change requires archive + changelog.
- [ ] Regenerate bundles — `node scripts/upgrade-design-tree-loop.mjs` and `node scripts/upgrade-alignment-page.mjs` — propagate both convention edits into every participating skill's `DESIGN-TREE-LOOP.md` / `ALIGNMENT-PAGE.md`.
- [ ] Confirm no other SKILL.md hard-codes the assemble-stop framing — grep `state-model` and siblings; §5 convention edit should cover them, but verify none carry a bespoke `## 0c`-style block needing the same edit.
- [ ] Publish + manifest — `git add` all source/convention edits first, run `scripts/pack.sh refresh` (updates `.codex/skills/`, `.claude/skills/`, catalog), then `git add` regenerated manifest and stage source + manifest together (index-generated-manifest rule).
- [ ] Verify: `node scripts/upgrade-design-tree-loop.mjs --check` and `node scripts/audit-alignment-pages.mjs` both exit 0.
- [ ] Verify: contract grep — `rg -n "assemble-ready|Progress Handoff Block|whole-set alignment review|Invoke With YAML" packs/product-design/codex/ux-variations/SKILL.md docs/design-tree-loop-convention.md` shows assemble-ready no longer co-occurring with repeat-command/second-YAML framing.
- [ ] Verify (acceptance): read regenerated `.codex/skills/ux-variations/{SKILL,DESIGN-TREE-LOOP,ALIGNMENT-PAGE}.md` and confirm the assemble stop with a `review` page yields a handoff that (a) leads with the HTML review path, (b) presents only the compiled response YAML, (c) has no repeat-`$ux-variations` framing or `Exact next command` line, (d) retains completed-count + durable-cursor lines.
- [ ] Commit source + regenerated bundles + manifest together on `master` and push; scope commit to these files only (concurrent-session working-tree caution).
