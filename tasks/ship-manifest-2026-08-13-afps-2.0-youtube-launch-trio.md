...
- The same focused command after `AFPS-CANARY-001` remediation — passed, 3 files / 28 tests, including alternate checkpoint, restored alignment producer, reversible upload-ready wording, and structurally valid publish/upload next-move mutations.
...
- Remediation: derive legacy stop/page totals from source-confirmed stop markers; derive active routine stops, unique checkpoint IDs/owners, and review-page producers from active source plus bundle existence; reject direct publish/upload/schedule and authenticated/Studio/public-visibility next moves while preserving local upload-ready preparation; add mutation fixtures and focused tests. A focused read-only re-review of the amended exact head is required before merge.
