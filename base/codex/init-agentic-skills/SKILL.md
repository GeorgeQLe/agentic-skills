...
1. If `.agents/project.json` exists, run `scripts/pack.sh refresh` to recreate local pack skill roots from committed project state, then report the project type, enabled packs, and installed skills. If `refresh` fails, report the failure and the exact command the user can retry.
2. If `.agents/project.json` is missing, inspect the repository before asking the user:
   ...
   - Present a plain-text **Pack Decision Checkpoint** (template below): show 2-4 numbered choices, mark the recommended choice, explain the tradeoff for each, and ask the user to reply with a number, exact pack list, `status`, or `cancel`. Include kanban only as an explicit opt-in when there is evidence of PoketoWork usage or the user asked for kanban.
