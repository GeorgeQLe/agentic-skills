...
doctor() {
  ...
  echo "Base skill install drift (~/.claude/skills, ~/.codex/skills):"
  ...
  $found || echo "  (no managed base skill installs found)"
  if $any_stale; then
    echo ""
    echo "Fix: /init-agentic-skills update  (re-copies base skills from this checkout)"
    return 1
  fi
  return 0
}
