...
3. Explain pack access separately:
   ...
   - In the project that needs domain workflows, run `$pack` for guided setup, `$pack install <pack-or-skill>` inside Codex for an explicit pack or individual pack skill, or `npx skillpacks install <pack-or-skill>` from the project shell.
