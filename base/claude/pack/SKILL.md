...
- `npx skillpacks install customer-discovery` from the project shell installs only the `customer-discovery` skill from `business-discovery`, not the whole pack.
- `npx skillpacks install code-review customer-discovery` from the project shell installs the `code-review` pack (all skills) plus the individual `customer-discovery` skill.
