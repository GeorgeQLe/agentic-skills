...
function skillSpecificGates(skillName, skillPath) {
  ...
  const rules = {
    ...
    "build-ui-screens": `**Screen-batch gates.** Render the approved UI branch, the per-flow-step element-batch plan, each batch's visual checkpoint and minimum-UI stop, the build ledger entries (flow_step, elements_added, status), the fake/fixture data boundary, deferred infrastructure, review evidence, and the handoff to logic-wiring as gates. ${outputGateDedup}`,
