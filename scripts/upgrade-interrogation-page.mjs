...
const INTERROGATION_AREAS = {
  ...
  "enterprise-icp": `**Interview areas the confidence gate covers.** Before advancing to scope approval, cover: enterprise stakeholder roles and which are relevant or out of scope; buying stages from evaluation through pilot and rollout; deal-killer requirements such as SSO, compliance, audit logs, RBAC, SLAs, data residency, and security-review readiness; onboarding complexity and migration/training needs; enterprise requirements delta versus the startup ICP; champion enablement and champion-risk assumptions; procurement reality, budget timing, approval chain, and source of budget; land-and-expand signals observed versus strategic recommendations deferred downstream; enterprise segmentation by tier and segment-specific deal-killers; source coverage gaps; and unknowns that could change the research scope. Open inputs must let the user add or remove stakeholders, correct buying-stage assumptions, identify must-have requirements, supply procurement or champion realities the agent cannot infer, and waive segmentation only when it is not relevant.`,
