...
describe("benchmark results matrix", () => {
  it("is generated from persisted benchmark reports and separates coverage from results", () => {
    ...
    expect(matrix).toMatch(
      /\| `pack` \| Codex \| `tests\/benchmarks\/runs\/pack-codex-[^/]+\/report\.json` \| 3 \| 100% \| 86\.4% \| none \| graded \|/,
    );
    expect(matrix).toMatch(
      /\| `exec` \| Codex \| `tests\/benchmarks\/runs\/exec-codex-[^/]+\/report\.json` \| blocked\/incomplete \| Report exists with zero total and evaluated runs\. Do not count as benchmarked\. \|/,
    );
