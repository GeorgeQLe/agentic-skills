...
skill: ship-end
agent: codex
captured_at: 2026-06-12T16:00:19-04:00
...
# Resolve Generated Skill-Root Shipping Blocker

## Summary

Unblock `$ship-end` by treating tracked local skill install files as generated artifacts. The repo currently tracks only two generated-root files:

- `.codex/skills/skill-interview/SKILL.md`
- `.claude/skills/skill-interview/SKILL.md`

Only the Codex copy is currently modified, but both should be removed from Git tracking so future pack refreshes do not block shipping.

## Key Changes

- Remove the two tracked generated skill files from Git index while keeping local files in place:
  - `git rm --cached .codex/skills/skill-interview/SKILL.md .claude/skills/skill-interview/SKILL.md`
- Update ignore rules narrowly:
  - Keep existing `.claude/skills/`
  - Replace or supplement broad `.codex` ignore with explicit `.codex/skills/` if needed, so generated skills stay ignored without accidentally hiding future project config under `.codex`.
- Update task docs and ship manifest to record that the generated-root blocker was resolved by untracking generated install artifacts.
- Commit this hygiene fix separately before attempting to ship the larger dirty tree.

## Test Plan

- Confirm no generated-root files remain tracked:
  - `git ls-files .codex/skills .claude/skills`
- Confirm generated local skill files still exist on disk:
  - `find .codex/skills .claude/skills -maxdepth 2 -name SKILL.md -print`
- Confirm ignore behavior:
  - `git check-ignore -v .codex/skills/skill-interview/SKILL.md .claude/skills/skill-interview/SKILL.md`
- Confirm no whitespace issues:
  - `git diff --check`
- Commit the hygiene-only boundary, then rerun `$ship-end` to handle the broader validation/remediation boundary.

## Assumptions

- `.codex/skills/**` and `.claude/skills/**` are generated local install roots, not canonical source.
- Canonical skill sources remain under `global/**`, `packs/**`, and other repo-managed source locations.
- The broader dirty tree should not be committed as part of this hygiene fix.
