...
skill: pack
agent: codex
captured_at: 2026-06-10T20:05:45-04:00
...
# Roadmap Plan: Skillpacks CLI Routing Remediation

## Summary

Update `tasks/roadmap.md` to add a new remediation phase for migrating active skill routing language to include the published npm CLI path: `npx skillpacks install <pack-or-skill>` and `npx skillpacks install-deck <deck>`. Also update `tasks/todo.md` so the next executable work is the first remediation planning/execution slice, not the whole 220-skill migration at once.

## Key Changes

- Add a new top-level roadmap section: `Current Implementation - Skillpacks CLI Routing Remediation`.
- Base the scope on `research/skillpack-cli-routing-audit.md`: 220 active skills need npm-aware routing wording; 14 global routing/install skills are P1.
- Split remediation into phases:
  1. Canonical wording and validation design.
  2. P1 global routing/install skills.
  3. Repeated `Pack Availability Guard` boilerplate across pack skills.
  4. Bespoke follow-up route sections in high-traffic workflow skills.
  5. Final validation, Skills Showcase refresh if any `SKILL.md` metadata/content changes require it, commit, and push.
- Explicitly require skill versioning rules for every changed `SKILL.md`: archive old version with `scripts/skill-archive.sh`, bump frontmatter `version`, and update each skill `CHANGELOG.md` where applicable.

## Implementation Details

- Roadmap update should not perform the skill remediation itself; it should only schedule it.
- `tasks/todo.md` should become the immediate execution slice: define canonical dual-route wording and decide/implement the validation rule that prevents future install-route text from omitting `npx skillpacks`.
- Acceptance criteria should include:
  - Roadmap references the audit report.
  - P1/P2/P3 remediation phases are sequenced and checkable.
  - Future implementation must preserve `/pack`, `$pack`, and `scripts/pack.sh` source-checkout routes while adding npm CLI alternatives.
  - Future implementation must distinguish pack installs from deck installs.

## Test Plan

- Run `git diff --check`.
- Verify `tasks/roadmap.md` includes the remediation phase and references `research/skillpack-cli-routing-audit.md`.
- Verify `tasks/todo.md` contains a self-contained first execution slice for canonical wording plus validation.
- No source tests are required for this roadmap-only update.

## Assumptions

- The roadmap should schedule full remediation of all 220 flagged active skills, not just the P1 global skills.
- The remediation should be phased to avoid one oversized commit touching hundreds of skill files at once.
- Existing in-agent routes remain valid; the migration adds npm CLI alternatives rather than replacing `/pack`, `$pack`, or `scripts/pack.sh`.
