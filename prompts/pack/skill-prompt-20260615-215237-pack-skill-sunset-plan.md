...
skill: pack
agent: codex
captured_at: 2026-06-15T21:52:37-04:00
...
# Pack Skill Sunset Alignment Page Plan

## Summary
Produce a review-state HTML alignment page at `alignment/pack-skill-sunset-plan.html` that gives a decision-ready roadmap for fully sunsetting the `pack` skill. The page should explain current dependencies, define the target replacement model, list implementation phases, and include approval gates before any actual sunset work begins.

Use `document` visual tier, `utility` alignment category, dark-mode styling, top in-flow table of contents, section feedback controls, bottom compile controls, TTS include, and central `alignment/index.html` registration.

## Key Changes
- Create one self-contained HTML page showing:
  - Current state: active `/pack` / `$pack` skill, `scripts/pack.sh`, npm `skillpacks`/`gskp`, root instruction dependencies, docs/tests references.
  - Sunset definition: what “fully sunset” means, including whether `pack` skill is removed, deprecated, or kept as a thin compatibility shim.
  - Readiness blockers: Node ownership for `recommend`, `which`, `install-deck`; root instruction fallback replacement; docs/test updates; package test health.
  - Proposed phased path: assessment, Node parity, agent-instruction migration, docs/test cleanup, deprecation/removal, release verification.
- Include decision gates for:
  - Sunset target: remove `pack` skill vs deprecate and retain shell compatibility.
  - CLI ownership: port remaining shell-backed commands vs document them as permanent compatibility surfaces.
  - User-facing route: standardize on `npx skillpacks` / `gskp` and remove `/pack` / `$pack` guidance.
  - Approval to proceed with implementation after the page is reviewed.
- Update `alignment/index.html` to link the new page with date metadata and `[doc]` badge.
- If this is executed as a skill-triggered task, capture the prompt under `prompts/pack/` before writing artifacts.

## Implementation Steps
- Inspect current sources before writing the page:
  - `base/{codex,claude}/pack/SKILL.md`
  - `scripts/pack.sh`
  - `packages/skillpacks/src/cli/*`
  - root `AGENTS.md` / `CLAUDE.md`
  - package compatibility docs and tests
- Render the page as a full review artifact, not a short summary. Include evidence tables with repo paths and observed dependency type.
- Add section-level feedback controls and response YAML behavior following `docs/alignment-page-convention.md`.
- Keep the page in `review` status. Do not mark it `confirmed` or perform sunset mutations until compiled approval YAML is received.

## Test Plan
- Run `node scripts/audit-alignment-pages.mjs`.
- Run `npx skillpacks alignment pages inject-tts alignment/pack-skill-sunset-plan.html` if TTS is missing.
- Run `git diff --check`.
- Optionally open the page with `npx skillpacks alignment pages open alignment/pack-skill-sunset-plan.html --browser auto`.

## Assumptions
- The page is a planning/review artifact only; it will not remove or edit the `pack` skill.
- Default recommendation in the page should be conservative: do not fully sunset until remaining shell-backed CLI surfaces are resolved and package verification is green.
- The implementer should not invent new CLI behavior while producing the page; the page should describe the work needed, not implement it.
