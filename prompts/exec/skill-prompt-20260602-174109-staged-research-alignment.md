...
skill: exec
agent: codex
captured_at: 2026-06-02T17:41:09-04:00
...
# Split Research Skills Into Staged Alignment Workflow

## Summary
Yes, we should split heavy research skills into a three-stage workflow. It reduces context pressure, gives the user a cleaner review loop, and keeps canonical research files from being written before approval.

Yes, this should be planned in `tasks/roadmap.md` and `tasks/todo.md`, then executed through `$exec --phase` or sequential `$exec` steps. This is cross-cutting skill-contract work touching many packs, generated alignment bundles, tests, versioning, changelogs, and showcase data. Individual future research runs should not be wrapped in `$exec`; they should invoke the research skill directly.

## Key Changes
- Add a shared staged-research contract to the canonical alignment convention and research skill patterns:
  - Stage 1: research + clarification, write non-canonical working packet.
  - Stage 2: consume working packet, build `review` HTML alignment page, stop for user feedback or final compiled YAML.
  - Stage 3: consume final YAML, apply approved edits, archive the working packet, confirm the HTML page, and write canonical research.
- Use working paths, not canonical output paths:
  - Flat mode: `research/_working/preliminary-<skill>-research.md`
  - Product-path mode: `research/{slug}/_working/preliminary-<skill>-research.md`
  - Archive on finalization to `docs/history/archive/YYYY-MM-DD/HHMMSS/<original-working-path>`, then remove the active working file.
- Keep canonical outputs unchanged:
  - ICP still finalizes to `research/icp.md` or `research/{slug}/icp.md`.
  - Search logs may remain as supporting evidence where current skill contracts already require them.
- Update alignment page behavior:
  - `review` pages render the full preliminary packet plus evidence matrix, assumptions, source gaps, proposed canonical file changes, and approval gates.
  - Feedback-only YAML revises the page/packet and remains in Stage 2.
  - Final compiled YAML triggers Stage 3 only when no unresolved negative feedback or clarification remains.
  - `confirmed` pages preserve approval records and no longer show active gate controls.

## Implementation Plan
- First create a roadmap/todo phase named `Staged Research Alignment Workflow`.
- Archive and version-bump affected skill contracts before behavioral edits.
- Update the shared convention in `CLAUDE.md`, then run `node scripts/upgrade-alignment-page.mjs` to regenerate owned `ALIGNMENT-PAGE.md` bundles.
- Pilot the full contract on mirrored ICP skills first, then apply the shared pattern to active report-first research skills with canonical research outputs.
- Update `research-roadmap` so queued research routes recommend direct skill commands, not `$exec`.
- Preserve the existing lesson: `$exec` is for project execution phases, not a wrapper around named research skills.

## Test Plan
- Add focused layer1 coverage that proves:
  - Stage 1 writes only `_working/preliminary-...` artifacts, not canonical research.
  - Stage 2 alignment pages can be generated from the preliminary packet.
  - Feedback-only YAML keeps the workflow in review.
  - Final compiled YAML archives the preliminary packet, confirms the page, and writes canonical research.
  - Research-roadmap and next-step routing do not prepend `$exec` to named research skills.
- Run:
  - `node scripts/upgrade-alignment-page.mjs --dry-run`
  - `bash scripts/skill-archive-audit.sh --strict`
  - focused layer1 tests for staged research and alignment lifecycle
  - `bash scripts/validate-skills-showcase-data.sh`
  - `git diff --check`

## Assumptions
- The preliminary research packet is allowed before approval because it is explicitly non-canonical working state, not a downstream source of truth.
- ICP is the pilot, but the final implementation should cover all active report-first research skills with canonical research outputs.
- Implementation should be shipped through the normal `$exec` flow because this is repo workflow infrastructure, not a single research invocation.
~~~
