...
skill: exec
agent: codex
captured_at: 2026-06-15T21:08:07-04:00
...
# Fix Layer1 Skill Alignment Routing Audit

## Summary

Fix `layer1/skill-alignment-routing-audit.test.ts` by correcting false positives in `scripts/skill-alignment-routing-audit.mjs`, not by mass-editing 44 active skill contracts. The active skills already express the intended approval-gated workflow; the audit is too literal about one exact phrase and too broad when matching prohibitive `/exec` language.

## Key Changes

- Update the audit’s direct exec handoff detection to ignore prohibitive sentences such as “Do not ... route to `/exec`”, while still failing positive handoffs like `Recommended next command: $exec`.
- Replace the brittle exact stop-contract check with a semantic contract check requiring:
  - `final compiled YAML`
  - `review`
  - an explicit stop/approval boundary before downstream routing
  - downstream routing blocked until approved artifacts are written or updated
- Add focused fixtures for the two false-positive shapes:
  - valid non-exec skill saying not to route to `$exec`/`/exec`
  - valid staged research skill using the current “approval request is the next action / no downstream routing language” wording
- Keep existing invalid fixtures failing:
  - positive `$exec` recommendation from a non-exec skill
  - review page followed by downstream routing without final compiled YAML approval

## Test Plan

- Run `node scripts/skill-alignment-routing-audit.mjs --fixtures tests/fixtures/skill-alignment-routing` and confirm only invalid fixtures produce findings.
- Run `node scripts/skill-alignment-routing-audit.mjs --report` and confirm `Alignment-routing findings: 0`.
- Run `pnpm --dir tests exec vitest run --project layer1 layer1/skill-alignment-routing-audit.test.ts`.
- Run `git diff --check`.

## Assumptions

- Do not version-bump or edit the 44 reported `SKILL.md` files; this is an audit false-positive fix, not a skill behavior change.
- No generated package/showcase refresh is needed unless verification shows the changed audit script is packaged into generated artifacts.
- If implementation is requested, record the visible prompt/task notes per repo workflow before mutating files.
