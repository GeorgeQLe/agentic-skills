...
skill: investigate
agent: codex
captured_at: 2026-06-17T09:46:45-0400
source: user-invocation
prompt_scope: visible-user-invocation
---

```text
$investigate The current repo contract still models Pattern A continuation as explicit parent re-invocation plus pasted YAML. docs/research-session-loop-convention.md specifically says bare YAML is data and
  should not be relied on to auto-call the skill. That is the part that conflicts with your preferred model.

  I agree with your revised direction: the compiled YAML should carry enough routing context for a fresh session to know the parent orchestrator and any inline framework context. The better model is
  a self-routing continuation payload, not direct framework invocation.
  ...
  Key principle: the YAML can route the fresh agent to the parent skill, but the parent still owns interpretation, state resolution, artifact writing, archiving, and whether to load a framework
  subskill inline. The YAML should never expose a path-shaped child command as the user-facing route.
```
