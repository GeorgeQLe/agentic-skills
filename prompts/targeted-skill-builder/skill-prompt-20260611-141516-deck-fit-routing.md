...
agent: codex
captured_at: 2026-06-11T14:15:16-04:00
...
# Add Deck-Fit Routing To `idea-scope-brief`

## Summary
Update `idea-scope-brief` so completed idea briefs recommend the closest workflow deck, preferring user-saved config decks when present and falling back to the five canonical decks: `vard`, `ord`, `business-afps`, `devtool-afps`, `game-afps`.

## Key Changes
- Use `targeted-skill-builder` workflow for a narrow existing-skill update.
- Archive current `global/codex/idea-scope-brief/SKILL.md` with `scripts/skill-archive.sh global/codex/idea-scope-brief`, then bump `version:` from `v0.15` to `v0.16`.
- Add a `Deck Fit Handoff` rule to `idea-scope-brief`:
  - Read canonical deck metadata from `docs/decks.md` / skillpacks manifest concepts.
  - Read user-saved deck candidates from repo config first, specifically `.agents/project.json` fields `saved_decks` or `decks` if present.
  - Rank candidates by domain, tempo, and concept signals.
- Add explicit default routing examples:
  - Game/playable entertainment concept -> `game-afps`.
...
- Update `## Next Steps` so the primary recommendation is the best deck route when confidence is high, using `npx skillpacks install-deck <deck>` for canonical decks. Customized/saved decks should emit explicit pack install guidance, not `install-deck`, unless they preserve a canonical slug.
- Keep downstream research routing as secondary context, not the primary command, once a deck recommendation is available.
- Update `CHANGELOG.md` with `v0.16`.

## Tests And Verification
- Run focused text checks for the new behavior:
...
- Because `SKILL.md` behavior changes, refresh and validate showcase data:
...
## Assumptions
- “Own saved decks” means repo/config-file decks, not browser localStorage, unless a future task defines a backend/export format.
- No new deck runtime primitive is added; this is skill guidance only.
- Canonical deck installs must use `npx skillpacks install-deck <deck>`, not `$pack install <deck>`.
- Prompt history capture should be included during implementation because this turn invoked skill work, but it was not written during Plan Mode.
```

# Visible User-Provided Skill Context
